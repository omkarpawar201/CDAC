...
public class ProductDaoImpl implements ProductDao {
	...
	static PreparedStatement getProduct, deleteProduct, addProduct, findProduct, findCategory;
	...
	static {
		try {
			...
			findCategory = conn.prepareStatement("select distinct cid from product1");
	...
	public List<Integer> getAllCategory() {
		...
		try {
			ResultSet rst = findCategory.executeQuery();
			while (rst.next()) {
				lst.add(rst.getInt(6));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
