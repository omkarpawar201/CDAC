...
public class ProductDaoImpl implements ProductDao {
	...
	public List<Product> findByCatId(int cid) {
		List<Product> plist = jdbcTemplate.query("select * from product where cid=?", new Object[] { cid },
				(rs, numrows) -> {
					Product p = new Product();
					p.setPid(rs.getInt(1));
					p.setPname(rs.getString(2));
	...
	public boolean updateById(int pid, int qty, double pr) {
		int n = jdbcTemplate.update("update product set qty=?,price=? where pid=?", new Object[] { qty, pr, pid });
		return n > 0;
	}
